...
export default function ProductsList({ initialProducts, initialPagination, categories }: ProductsListProps) {
  ...
  const renderEditableCell = (product: ProductWithCategory, field: keyof Product) => {
    if (editingProduct?.id === product.id) {
      if (field === 'isActive') {
        // Handle tri-state for checkbox: indeterminate becomes false
        return <Checkbox checked={!!editingProduct[field]} onCheckedChange={(checked) => setEditingProduct({...editingProduct, [field]: checked === true})} />;
      }
        ...
        <TableBody>
          {products.map((p) => (
            <TableRow key={p.id}>
              ...
              <TableCell>{editingProduct?.id === p.id ? renderEditableCell(p, 'isActive') : String(p.isActive)}</TableCell>
              <TableCell>
